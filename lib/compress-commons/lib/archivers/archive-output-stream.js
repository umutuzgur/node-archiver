...
export default class ArchiveOutputStream extends Transform {
  ...
  _appendFakeFile(ae, source, callback) {
    // scaffold only
  }
  ...
  _appendCompressedStream(zae, source, callback) {
    // scaffold only
  }
  ...
  entry(ae, source, callback) {
    ...
    if (Buffer.isBuffer(source)) {
      ...
      if (this.dry) {
        this._appendFakeFile(ae, source, callback);
      } else {
        // If the compressed size already exists, append with compressed
        if (ae.csize) {
          this._appendCompressedStream(zae, source, callback)
        } else {
